            ...
            <TableRow key={comanda.id}>
                ...
                <TableCell>
                    ...
                    <ActionButtons onView={handleView} onEdit={user?.grupo === USER_GROUPS.ADMINISTRADOR ? handleEdit : null} onDelete={user?.grupo === USER_GROUPS.ADMINISTRADOR ? handleDelete : null} item={comanda} >
                        <IconButton size="small" color="success" title="Adicionar Consumo" sx={{ width: 40, height: 40, '&:hover': { backgroundColor: 'success.light', color: 'white' } }} onClick={() => handleAddItem(comanda)} >
                            <AddCircle fontSize="small" />
                        </IconButton>
                        {user?.grupo === USER_GROUPS.ADMINISTRADOR && (
                            <IconButton size="small" color="warning" title="Cancelar Comanda" sx={{ width: 40, height: 40, '&:hover': { backgroundColor: 'warning.light', color: 'white' } }} onClick={() => handleCancel(comanda)} >
            ...
            <Card key={comanda.id} sx={{ mb: 2 }}>
                <CardContent>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                        <IconButton size="small" color="primary" title="Visualizar" sx={{ width: 40, height: 40, '&:hover': { backgroundColor: 'primary.light', color: 'white' } }} onClick={() => handleView(comanda)} >
                            <Visibility fontSize="small" />
                        </IconButton>
                        {user?.grupo === USER_GROUPS.ADMINISTRADOR && (
                            <IconButton size="small" color="secondary" title="Editar" sx={{ width: 40, height: 40, '&:hover': { backgroundColor: 'secondary.light', color: 'white' } }} onClick={() => handleEdit(comanda)} >
                                <Edit fontSize="small" />
                            </IconButton>
                        )}
                        {user?.grupo === USER_GROUPS.ADMINISTRADOR && (
                            <IconButton size="small" color="error" title="Excluir" sx={{ width: 40, height: 40, '&:hover': { backgroundColor: 'error.light', color: 'white' } }} onClick={() => handleDelete(comanda)} >
